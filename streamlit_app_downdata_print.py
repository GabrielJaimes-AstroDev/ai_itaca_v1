...
with col2:
    ...
    st.markdown('<p class="main-title">AI-ITACA | Molecular Spectrum Analyzer</p>', unsafe_allow_html=True)
    st.markdown('<p class="subtitle">Artificial Intelligence Integral Tool for AstroChemical Analysis</p>', unsafe_allow_html=True)
...
# === CONFIGURACIÓN === (el resto del código original permanece igual)
...
def set_white_text():
    ...
    function styleFileUploader() {
        // Texto "Browse files"
        const browseTexts = document.querySelectorAll('.stFileUploader label div div');
        browseTexts.forEach(el => {
            el.style.color = 'white !important';
            el.style.fontSize = '14px !important';
        });
        
        // Texto del comando
        const cmdText = document.querySelector('.stAlert code');
        if (cmdText) {
            cmdText.style.color = 'white !important';
        }
    }
    
    // Ejecutar inmediatamente y cada segundo por si el componente tarda
    styleFileUploader();
    setInterval(styleFileUploader, 1000);
    </script>
    """
    html(js_code, height=0)

# Llamar a la función antes de crear los componentes
set_white_text()

# Tu file uploader en el sidebar
