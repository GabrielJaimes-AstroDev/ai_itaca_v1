...
def download_models_from_drive(folder_url, output_dir):
    ...
    return model_files, data_files, False
...
if models_downloaded:
    st.sidebar.success("✅ Detection Models already downloaded")
else:
    with st.sidebar:
        with st.spinner("📥 Downloading Models for detection..."):
            pass
                    ...
                    if not os.path.exists(data_file):
                        ...
                        with tab0:
                            if results.get('best_match'):
                                st.subheader("Interactive Summary Comparison")
                                ...
                                # Añadir mejor coincidencia - CORREGIDO según core_functions.py
                                fig.add_trace(go.Scatter(
                                    x=results['best_match']['x_synth'],  # Cambiado de 'frequency' a 'x_synth'
                                    y=results['best_match']['y_synth'],  # Cambiado de 'spectrum' a 'y_synth'
                                    mode='lines',
                                    name='Best Match',
                                    line=dict(color='red', width=2)
                                ))
                                
                                # Añadir líneas verticales para picos coincidentes
                                if results['best_match'].get('matches'):
                                    for match in results['best_match']['matches']:
                                        if 'input_peak' in match and 'synth_peak' in match:
                                            fig.add_vline(
                                                x=match['input_peak']['center'],
                                                line=dict(color='blue', width=1, dash='dash'),
                                                opacity=0.5
                                            )
                                            fig.add_vline(
                                                x=match['synth_peak']['center'],
                                ...
                                fig.update_layout(
                                    title='Interactive Spectrum Comparison',
                                ...
                                # Formatear el eje x para mostrar GHz
