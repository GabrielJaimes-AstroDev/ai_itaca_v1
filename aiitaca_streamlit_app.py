...
def download_models_from_drive(folder_url, output_dir):
    ...
    try:
        ...
        # Extraer el ID de la carpeta de Google Drive
        folder_id = re.search(r'folders/([a-zA-Z0-9-_]+)', folder_url).group(1)
        if not folder_id:
            raise ValueError("Invalid Google Drive folder URL")

        # Crear directorio temporal para wget
        temp_download_dir = os.path.join(output_dir, "temp_wget")
        os.makedirs(temp_download_dir, exist_ok=True)

        # Comando wget para descargar toda la carpeta
        cmd = [
            'wget',
            '--quiet',
            '--show-progress',
            ...
            '-O', os.path.join(temp_download_dir, "models.zip")
        ]

        # Ejecutar wget y capturar el progreso
        ...
        # Procesar la salida para mostrar progreso
        while True:
            output = process.stderr.readline()
            if output == '' and process.poll() is not None:
                break
            if output:
                # Extraer porcentaje de progreso de la salida de wget
        ...
        progress_text.text("📦 Extracting downloaded files...")
        progress_bar.progress(95)
        
        # Aquí necesitarías implementar la lógica para descomprimir el archivo ZIP
        # Por ejemplo, usando zipfile o shutil
        
        # Mover los archivos al directorio final
        for filename in os.listdir(temp_download_dir):
            if filename.endswith('.keras') or filename.endswith('.npz'):
                shutil.move(
