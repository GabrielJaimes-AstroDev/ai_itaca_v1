...
def find_input_file(filepath):
    ...
    for ext in ['', '.spec', '.fits', '.txt', '.dat']:
        test_path = f"{filepath}{ext}"
        if os.path.exists(test_path):
            return test_path
    
    raise FileNotFoundError(f"No se encontró el archivo {filepath} (probado con extensiones: '', '.spec', '.fits', '.txt', '.dat')")

def read_spec_file(filepath):
    """Lee un archivo .spec y devuelve frecuencias e intensidades"""
    try:
...
def read_fits_file(filepath):
    """Lee un archivo .fits y devuelve frecuencias e intensidades combinadas"""
    ...
    # Concatenar y ordenar todos los datos
    ...
    hdul.close()
    return combined_freqs, combined_intensities
...
def process_input_file(filepath):
    ...
    if filepath.lower().endswith('.fits'):
        freq, spec = read_fits_file(filepath)
    else:  # Asumir que es un archivo .spec o similar
        freq, spec = read_spec_file(filepath)
        # Intentar extraer parámetros del encabezado
        with open(filepath, 'r', encoding='utf-8') as file:
            first_line = file.readline().strip()
        if first_line and not first_line.startswith(("//", "#")):
            input_params = re.search(r'logn[\s=:]+([\d.]+).*tex[\s=:]+([\d.]+)', first_line.lower())
            if input_params:
                try:
