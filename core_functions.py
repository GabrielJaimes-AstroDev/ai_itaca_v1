...
DEFAULT_CONFIG = {
    ...
    'peak_matching': {
        'sigma_threshold': 1,          # Para detect_peaks
        'sigma_emission': 1.5,         # Para detect_emission_regions
        'window_size': 3,              # Para detect_emission_regions
        'fwhm_ghz': 0.08,
        'tolerance_ghz': 0.2,
...
def find_input_file(filepath):
    # Si es un archivo subido a través de Streamlit (ya existe)
    if os.path.exists(filepath):
        return filepath
    
    # Si no existe, probamos agregando extensiones comunes
    for ext in ['', '.txt', '.dat']:
...
def plot_individual_matches(new_x, input_spec, matches, fig=None):
    ...
    # Asegurar que matches sea una lista
    ...
    # Crear figura con subplots verticales (2 filas por match: 1 para el espectro, 1 para el residual)
    fig, axes = plt.subplots(n * 2, 1, figsize=(14, 5 * n), gridspec_kw={'height_ratios': [3, 1] * n})

    if n == 1:
        axes = [axes]  # en caso de que solo haya un match
