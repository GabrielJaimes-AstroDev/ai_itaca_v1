...
# === CONFIGURATION ===
GDRIVE_FOLDER_URL = "https://drive.google.com/drive/folders/1PyAGVOum6MWE_2PDqysvrr_dg_acg-v8?usp=sharing"
...
def download_models_from_drive(folder_url, output_dir):
    ...
    try:
        progress_bar = st.progress(0, text="📥 Downloading models from Google Drive...")
        ...
        # Actualizar progreso
        progress_bar.progress(50, text="📥 Downloading models from Google Drive...")
        
        model_files = [f for f in os.listdir(output_dir) if f.endswith('.keras')]
        data_files = [f for f in os.listdir(output_dir) if f.endswith('.npz')]
        
        progress_bar.progress(100, text="✅ Models downloaded successfully!")
        st.toast("Models downloaded successfully!", icon="✅")
        ...
        progress_bar.progress(100, text="❌ Error downloading models")
        st.error(f"❌ Error downloading models: {str(e)}")
        return [], [], False
    finally:
        # Pequeña pausa para que se vea el progreso al 100%
        import time
        time.sleep(0.5)
        progress_bar.empty()
...
if input_file is not None:
    ...
    if not model_files:
        ...
        if analyze_btn:
            try:
                # Crear un contenedor para la barra de progreso
                ...
                if not os.path.exists(data_file):
                    ...
                    progress_bar.progress(100, text="Error: Training data not found")
                ...
                # Paso 3: Analizar espectro
                progress_bar.progress(30, text="Analyzing spectrum...")
                ...
                results = analyze_spectrum(
                    ...
                    config, mol_name, progress_callback=update_progress
                )
